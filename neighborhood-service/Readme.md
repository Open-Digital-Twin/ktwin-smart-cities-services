...
This service implements the Neighborhood business logic for Smart Grids DTDL use case.

1. Calculate the different Air quality level of the area.
...
```bash
docker build -t ktwin/neighborhood-service:0.1 .
docker compose up -d
```
...
```bash
kind load docker-image ktwin/neighborhood-service:0.1
```
